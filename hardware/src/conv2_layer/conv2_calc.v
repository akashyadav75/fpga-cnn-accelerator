...
module conv2_calc
    ...
    assign conv_out_calc = calc_out[19:6]; // 14bit
        ...
        // Fixed: simple 4-cycle delay shift register matching the
    // 4-stage MAC pipeline (Stage1 -> Stage2 -> Stage3 -> Stage4).
    // The old toggle FSM produced a 50% duty-cycle square wave
    // which caused the FC layer to see twice as many valid beats.
    always @(posedge clk)
    begin
        if (~rst_n)
        begin
        ...
        begin
            valid_out_calc_tmp0 <= valid_out_buf;
